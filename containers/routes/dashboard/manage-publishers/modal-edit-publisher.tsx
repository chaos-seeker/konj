...
export function ModalEditPublisher({
  ...
  const onSubmit = async (data: PublisherFormValues) => {
    ...
    try {
      ...
      const result = await updatePublisher(publisher.slug, formData);
      if (!result.success) {
        throw new Error(result.error || "خطا در به‌روزرسانی ناشر");
      }
      await queryClient.invalidateQueries({ queryKey: ["publishers"] });
      toast.success(result.message || "ناشر با موفقیت به‌روزرسانی شد");
      form.reset();
      onOpenChange(false);
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : "خطا در به‌روزرسانی ناشر"
      );
    } finally {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>ویرایش ناشر</DialogTitle>
        </DialogHeader>
